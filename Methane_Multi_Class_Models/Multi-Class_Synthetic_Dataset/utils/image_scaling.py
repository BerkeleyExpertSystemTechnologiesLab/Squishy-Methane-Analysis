...
def scale_numpy_array(array, new_max, new_min=0, preserve_dtype=True):
    ...
    try:
        ...
        if current_max == current_min:
            ...
            scaled_array = new_min + (array - current_min) * (new_max - new_min) / (
...
def scale_and_save_array(
    array, new_max, new_min=0, output_path=None, preserve_dtype=True
):
    """
...
def scale_jpg_to_ppm(jpg_path, ppm_value, output_path=None, grayscale=False):
    ...
    try:
        ...
        if current_max == current_min:
            ...
            scaled_image = (
                (image_float - current_min) / (current_max - current_min)
            ) * ppm_value
        ...
        if array.dtype == np.float64 or array.dtype == np.float32:
            if normalize:
                ...
                if array_max > array_min:
                    array = (
                        (array - array_min) / (array_max - array_min) * 255
                    ).astype(np.uint8)
                else:
