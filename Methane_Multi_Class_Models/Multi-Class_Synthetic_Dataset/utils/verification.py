...
DEFAULT_PROCESSED_DATASET_PATH = "export_datasets/Processed_Dataset"
...
def analyze_numpy_array(numpy_array, array_name="Array"):
    ...
    try:
        ...
        print(
            f"  Non-zero pixels: {non_zero_pixels:,} / {total_pixels:,} ({non_zero_percentage:.1f}%)"
        )
...
def check_multiple_jpgs(jpg_paths):
    ...
    if successful > 0:
        ...
        max_values = [
            stats["max_value"] for stats in results.values() if stats is not None
        ]
        min_values = [
            stats["min_value"] for stats in results.values() if stats is not None
        ]
        ...
        if all(v <= 255 for v in max_values) and all(v >= 0 for v in min_values):
            print("All values are in valid 8-bit range (0-255)")
        else:
            print("Some values are outside 8-bit range (0-255)")
...
def verify_scaling_in_dataset(processed_dataset_path=DEFAULT_PROCESSED_DATASET_PATH):
    """
...
def verify_specific_videos(
    video_codes, processed_dataset_path=DEFAULT_PROCESSED_DATASET_PATH
):
    """
    ...
    for video_code in video_codes:
        ...
        print(
            f"  Total scaled JPGs found for video {video_code}: {len(video_scaled_jpgs)}"
        )
...
def check_numpy_array_values(npy_path):
    ...
    try:
        ...
        if len(numpy_array.shape) != 2:
            print(
                f"Warning: Expected single-channel PPM array (H, W), got shape {numpy_array.shape}"
            )
            return None
        ...
        if stats is not None:
            ...
            if max_val <= 255 and max_val > 150:
                print(
                    "WARNING: Values look like normalized 0-255 range, not PPM values"
                )
            elif max_val < 150:
                print("Values look like actual PPM values (reasonable range)")
            else:
                print("? Unusual value range - verify this is correct")
...
def verify_numpy_arrays_specific_videos(
    video_codes, processed_dataset_path=DEFAULT_PROCESSED_DATASET_PATH
):
    """
    ...
    for video_code in video_codes:
        ...
        print(
            f"  Total numpy arrays found for video {video_code}: {len(video_numpy_arrays)}"
        )
    ...
    if successful > 0:
        ...
        if len(high_values) > 0:
            print(
                f"  WARNING: {len(high_values)} files have max values > 100 PPM - may be normalized!"
            )
        else:
            print("  All PPM values are in reasonable range (< 100 PPM)")
        ...
        if len(very_low_values) > 0:
            print(
                f"  WARNING: {len(very_low_values)} files have very low max values (< 1 PPM)"
            )

    return results


def verify_all_numpy_arrays_in_dataset(
    ...
    if successful > 0:
        ...
        print(
            f"  Percentage of total files: {(high_ppm_count / len(all_numpy_arrays) * 100):.1f}%"
        )
        ...
        if high_ppm_count > 0:
            ...
            print("  No files found with PPM values > 150")
