...
import config  # Used on line 121 for function create_class_directories
...
def get_directory_files(directory_path, file_extensions=None):
    ...
    try:
        ...
        files_info = {"directory_path": directory_path, "total_files": 0, "files": []}
...
def create_class_directories(video_codes=None, num_classes=8, output_path=None):
    """
    ...
    # Use provided output path or default to global constant
    dataset_path = output_path if output_path else config.PROCESSED_DATASET_BASE
    ...
    try:
        for video_code in video_codes:
            ...
            video_dir = os.path.join(dataset_path, video_code)
