...
def add_ppm_data_to_classes(processed_dataset_path, plume_modeling_path):
    ...
    try:
        ...
        from utils.excel import convert_excel_to_csv
        ...
        for video_dir in os.listdir(processed_dataset_path):
            ...
            for class_num in range(8):
                ...
                try:
                    ...
                    if (
                        json_class_num is None
                        or leak_rate_scfh is None
                        or distance_m is None
                    ):
                        print(
                            f"  Class_{class_num}: Missing class number, leak rate, or distance in JSON"
                        )
                        processed_summary["failed_updates"] += 1
                        continue
                    ...
                    print(
                        f"  Class_{class_num}: Found columns - class: {class_col}, leak_rate: {leak_rate_col}, distance: {distance_col}, ppm: {ppm_col}"
                    )

                    if not all([class_col, leak_rate_col, distance_col, ppm_col]):
                        print(
                            f"  Class_{class_num}: Missing required columns in Excel data"
                        )
                        processed_summary["failed_updates"] += 1
                        continue
                    ...
                    matching_rows = df[
                        ...
                        & (
                            abs(df[leak_rate_col] - leak_rate_scfh) < 0.1
                        )  # Allow small tolerance for leak rate
                        & (
                            abs(df[distance_col] - distance_m) < 0.1
                        )  # Allow small tolerance for distance
                    ]

                    if len(matching_rows) == 0:
                        print(
                            f"  Class_{class_num}: No matching PPM data found for class {json_class_num}, leak_rate {leak_rate_scfh}, distance {distance_m}m"
                        )
                    ...
                    print(
                        f"  Class_{class_num}: Added PPM value {ppm_value} for class {json_class_num}, leak_rate {leak_rate_scfh}, distance {distance_m}m"
                    )
                    processed_summary["successful_updates"] += 1
                    processed_summary["total_classes"] += 1
