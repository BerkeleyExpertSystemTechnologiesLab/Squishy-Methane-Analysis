...
from utils.json_utils import find_class_json_file
...
def get_distance_mapping(excel_file_path):
    ...
    try:
        ...
        print(
            f"Successfully created distance mapping for {len(distance_mapping)} videos"
        )
        ...
        for video_number, distance in distance_mapping.items():
            ...
            for class_num in range(8):
                ...
                if not os.path.exists(class_dir):
                    print(
                        f"  Class_{class_num} directory not found for video {video_number}"
                    )
                    class_failures += 1
                    continue
                ...
                if not class_json_file:
                    print(
                        f"  Class_{class_num}: No JSON file found for video {video_number}"
                    )
                    class_failures += 1
                    continue
                ...
                try:
                    ...
                    print(
                        f"  Class_{class_num}: Added distance {distance}m for video {video_number}"
                    )
                    class_successes += 1

                except (json.JSONDecodeError, FileNotFoundError, KeyError) as e:
                    print(
                        f"  Class_{class_num}: Error updating JSON file for video {video_number}: {e}"
                    )
                    class_failures += 1
            ...
            if class_successes > 0:
                print(
                    f"Added distance descriptor for video {video_number}: {distance}m ({class_successes} classes updated)"
                )
            else:
                print(
                    f"Failed to add distance for video {video_number} (no classes updated)"
                )
